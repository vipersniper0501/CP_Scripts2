...
def Find_Groups(users_n_groups: bool):
    ...
    """
    def find_groups(users_n_groups: bool):
        output = sub.run(["powershell", "net localgroup"], stdout=sub.PIPE,
        ...
        if users_n_groups:
            for _, group in enumerate(groups):
                output = sub.run(["powershell", "net localgroup '"
                                  + group + "'"], stdout=sub.PIPE, check=True,
                                 text=True)
                output = output.stdout.split("\n")
                try:
                    user_names = []
            ...
            def CONFIRM():
                ...
                if self.adminyn == 'y':
                    ...
                    if Check_Password(self.Password1_input.text(),
                        ...
                        sub.run(f"sudo useradd -G users,admin,sudo -m -d"
                                " /home/{username} -p $(echo {passwd} | "
                                "openssl passwd -1 -stdin) {username}".split())
